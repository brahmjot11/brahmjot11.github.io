...
const elWipe = (e) => {
    ...
    setTimeout(() => {
        ...
        e.style.bottom = "initial";
        e.classList.remove('wpr');
...
const elState = (el, st, si) => {
    ...
    if (st === undefined || st === 0) {
        ...
        (el.id === "sr_intr") ? el.classList.add('rt') : elWipe(el);
    } else {
...
const nav = (p) => {
    ...
    if (cr !== 0) {
        ...
        if (cr === rt && cl === 0 && AppState === 1) {
            document.querySelectorAll('.vis').forEach((e) => { e.classList.remove('vis'); elWipe(e); });
